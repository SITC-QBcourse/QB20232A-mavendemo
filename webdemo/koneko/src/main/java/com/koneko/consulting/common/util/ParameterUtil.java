package com.koneko.consulting.common.util;
...
public class ParameterUtil {
	...
	public ParameterUtil(HttpServletRequest request, String uploadFile, String tempFile, String encoding, long maxSize,
			long fileMaxSize) {
	...
	public List<String> getUUIDName(String paramName) throws Exception {
		...
		for (String fileName : fileNames) { // 数组迭代
			uuidNames.add("koneko." + UUID.randomUUID() + "." + fileName.substring(fileName.lastIndexOf(".") + 1));
		}
	...
	public void saveUploadFile(String paramName, List<String> uploadFileNames) throws Exception {
		...
		for (int x = 0; x < fileNames.length; x++) {
			...
			File destFile = new File(
					this.request.getServletContext().getRealPath(this.uploadFile) + uploadFileNames.get(x));
